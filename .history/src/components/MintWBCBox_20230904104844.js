...
const MintWBCBox = () => {
    ...
    const { chainName, network, tokenContract, walletAddress } = useEthers();
    ...
    const shortWalletAddress = walletAddress ? `${walletAddress.slice(0, 5)}...${walletAddress.slice(-3)}` : null;
        ...
        const fetchCanMintWBC = useCallback(async () => {
            if (tokenContract) {
                try {
                    const canMintWBC = await tokenContract.checkIfUserCanMint(walletAddress);
                    console.log("succesfully called 'canMint' function in contract")
                    return canMintWBC;
        ...
        const canMintWBC = useContract(tokenContract, fetchCanMintWBC);
