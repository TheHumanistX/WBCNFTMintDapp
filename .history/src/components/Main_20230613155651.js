...
const useAllNftData = (contract, lastID) => {
    ...
    useEffect(() => {
      ...
      const fetchNftData = async (index) => {
        const { data: nft } = useContractRead(contract, "tokenURI", [index]);
        if (nft) {
          const url = ipfsUrl(nft);
          const response = await fetch(url);
          const json = await response.json();
          setAllNftData(oldArray => [...oldArray, json]);
        }
      };
  
      if (currentIndex <= lastID) {
        fetchNftData(currentIndex);
        setCurrentIndex(currentIndex + 1);
      }
    }, [currentIndex, lastID, contract]);
...
const Main = () => {
    ...
    const allNftData = useAllNftData(contract, lastID);
