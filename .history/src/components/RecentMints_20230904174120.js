...
const RecentMints = () => {
  ...
  const fetchEvents = useCallback(async (contract) => {
    ...
    if (crazyFacesContract) {
      ...
      const recentMints = mintEvents.length >= 5 ? mintEvents.slice(0, 5) : mintEvents;
  ...
  console.log('Recent Mints - after contractValues mintEvents: ', mintEvents, ' & recentMints: ', recentMints);
