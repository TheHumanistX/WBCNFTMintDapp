...
const MintBox = ({ mintContractAddress, mintContract, WBCContract, walletAddress }) => {
  ...
  const mintPrice = data ? parseInt(ethers.utils.formatEther(data)) : '';
  ...
  const WBCMintAmount = mintPrice ? mintPrice.toString() : '';
  const WBCAmountWithDecimals = ethers.utils.parseEther(WBCMintAmount);
  
  const callToMint = async () => {
    let approval;
    try {
        approval = await approve({ args: [mintContractAddress, WBCAmountWithDecimals] });
